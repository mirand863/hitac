...
label_data$acc_text_length <- nchar(as.character(label_data$Acc))
label_data$acc_text_position <- label_data$Acc - 5 - label_data$acc_text_length * 2
label_data$acc_text <- ifelse(label_data$Acc > 11, label_data$Acc, "")
...
# Make the plot
p <- ggplot(data, aes(x=as.factor(id), y=Acc, fill=Group)) +       # Note that id is a factor. If x is numeric, there is some space between the first bar
  ...
  coord_polar() +
  geom_text(data=label_data, aes(x=id, y=Acc+1, label=Method, hjust=hjust), color="black", fontface="plain",alpha=1, size=2.5, angle= label_data$angle, inherit.aes = FALSE ) +
  geom_text(data=label_data, aes(x=id, y=acc_text_position, label=acc_text, hjust=hjust), color="white", fontface="plain",alpha=1, size=2.5, angle= label_data$angle, inherit.aes = FALSE ) +
